...
static inline int cut_frame(struct cut_info *cut, int last_frame, int force, int *results, int *res_pos, int is_cut) {
    ...
    if (force || (frames >= CAL_FRAME_BY_TIME(FILE_CUT_MIN_MS))) {
		...
		if (cut->is_contain_active) {
			...
			results[*res_pos] = CAL_FRAME_BY_FRAME(cut->cut_begin_frame);
			results[*res_pos + 1] = CAL_FRAME_BY_FRAME(last_frame) - 1;
			*res_pos += 2;
		}
...
static inline int add_continued(struct cut_info *cut, int is_active, int *results, int *res_pos, int is_cut) {
    if (!is_active && cut->is_contain_active) {
        // 有响声，之后连续静音
        int diff = cut->previous_along_frames - CAL_FRAME_BY_TIME(FILE_CUT_SILENCE_AFTER_ACTIVE_MS);
        if (diff >= 0) {
            int res = cut_frame(cut, cut->current_frame, 0, results, res_pos, is_cut);
            if (res == 0) {
...
static inline int add_changed(struct cut_info *cut, int is_active, int *results, int *res_pos, int is_cut) {
    int frame = 0;
    if (is_active) {
        // 连续静音，之后遇到响声
        if (cut->previous_along_frames > CAL_FRAME_BY_TIME(FILE_CUT_SILENCE_BEFORE_ACTIVE_MS)) {
            ...
            int res = cut_frame(cut, c_frames, 0, results, res_pos, is_cut);
            if (res == 0) {
                frame = -1 * (c_frames);
            }
...
int cut_add_vad_activity(struct cut_info *cut, int is_active, int is_last, int *results, int *res_pos, int is_cut) {
    int res;
    if (is_last ||
        (cut->current_frame - cut->cut_begin_frame == CAL_FRAME_BY_TIME(FILE_CUT_MAX_MS))) {
        cut_frame(cut, cut->current_frame, is_last, results, res_pos, is_cut);
        res = -1 * cut->current_frame;
    } else if (cut->is_pervious_active == is_active) {
        res = add_continued(cut, is_active, results, res_pos, is_cut);
    } else {
        res = add_changed(cut, is_active, results, res_pos, is_cut);
        if (is_active) {
